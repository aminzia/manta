...
operator<<(std::ostream& os, AlignmentResult<ScoreType>& alignment)
{
    os << "AlignerResult: score: " << alignment.score << "\n"
       ...
       << "\tscore2: " << alignment.score2 << "\n"
       << "\talignment2: " << alignment.align2 << "\n";
    return os;
}
    ...
    BackTrace<ScoreType>& btrace2)
{
    ...
    else if(thisMax == btrace.max)
    {
        btrace2.max = thisMax;
        btrace2.refBegin = refIndex;
        btrace2.queryBegin = queryIndex;
        btrace2.isInit=true;
    }
}
    ...
    AlignmentResult<ScoreType>& result) const
{
    ...
    // capture 'second-best' alignment, including last observation of equally high score:
    BackTrace<ScoreType> btrace2;

    {
        ...
        for (SymIter refIter(refBegin); refIter != refEnd; ++refIter, ++refIndex)
        {
            ...
            // get backtrace info:
            {
                ...
                updateBacktrace(thisMax,refIndex+1,querySize,btrace,btrace2);
            }
        }
    }
    ...
    for (unsigned queryIndex(0); queryIndex<=querySize; queryIndex++)
    {
        ...
        updateBacktrace(thisMax,refSize,queryIndex,btrace,btrace2);
    }
...
#ifdef ALN_DEBUG
    log_os << "bt-start queryIndex: " << queryBegin << " refIndex: " << refBegin << " state: " << AlignState::label(btrace.state) << " maxScore: " << btrace.max << "\n";
#endif
    ...
    result.score2 = btrace2.max;
    if(result.score2 == result.score)
    {
        backTraceAlignment(queryBegin, queryEnd, refBegin, refEnd,
                querySize, btrace2, result.align2);
    }
}
